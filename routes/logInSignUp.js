...
router.get('/ajaxLogin', function(req, res){
	...
	let sqlQuery = 'SELECT * FROM users WHERE user_name = "'+ req.query.Name +'"'
	connection.query(sqlQuery, function(err, rows, fields){
		if(err){
			throw err
		}else{
			if(rows.length <= 0){
				...
				let sqlQuery = 'SELECT user_name, user_DOB, user_gender, user_id, user_password FROM users WHERE user_name = "'+ req.query.Name +'"'
				connection.query(sqlQuery, function(err, rows, fields){
					if(err){
						throw err
					}
...
router.get('/ajaxCheckLoggedIn', function(req, res){
	if(!req.query.currentToken || req.query.currentToken == 'token='){
		...
		let token = req.query.currentToken.split('=')[1]

		let sqlQuery = 'SELECT user_name, user_DOB, user_gender, user_id, user_password FROM users WHERE login_token = "'+token+'"'
		connection.query(sqlQuery, function(err, rows, fields){
			if(err){
				throw err
			}else{
